...
**Object i.d., zip code 2, zip name, existing, zip code 1, shape legend, shape area, geometry. It looks like object i.d. is an integer column, shape legend and shape area are number columns, geometry is a multipolygon column, while the rest are character columns.**
```{r}
...
**It lists out each location with an 11 digit zip code (2), the neighborhood/name of the location, what looks like some sort of year marker, a regular 5 digit zip code (1), followed by what look like the actual coordinates.**
```{r}
View(md_zips)
```
...
**There's a map of Maryland with outlines of each zip code**
```{r}
md_zips |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**It intalled the package and loaded it in**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's 18 new columns including geoid, the county names, and another multipolygon column**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There are 24 features and 17 fields. The first 10 features list out the Maryland counties and their geoid. The section below that appears to be a list of coordinates.**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's a map of Maryland with each county outlined**
```{r}
md_counties |>
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The zip code and counties maps are merged together**
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**20774 has the most forclosure notices in March of this year**
```{r}
...
foreclosure_zip_march_2023 |> arrange(desc(march_2023))
```
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The list of counties and coordinates now includes the March 2023 foreclosure data**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The zip code and county map is color coded with the number of foreclosure notices**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's a dataframe with the number of notices per county and another one with the population of the county**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's a shapefile that inlcudes population and coordinates of each county**
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**Some of the county names are different in the notices data and the shapefile**
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The "NAME" column is now "county" and the county names are uniform to the style in the notices data**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The dataset has the county names, the population and number of notices per county as well as the map coordinates**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's a "rate" column that calculates the percent of notices per 1000 - which shows that Charles County had the highest rate of notices**
```{r}
md_pop_with_foreclosures <- md_county_population |>
  left_join(md_county_notices, join_by(county)) |> 
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**There's a color coordinated map based on the number of notices per 1000 for each county**
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer**
**The previous map is now in a purple to yellow color gradient and the county names are gone**
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
...
**Charles county stands out a lot more than the previous map. It's also a lot more clear which surrounding counties come close to the rate of notices in Charles county. The counties directly surrounding Charles have pretty high rates along with the upper northeast and the midwest of Maryland.**
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +

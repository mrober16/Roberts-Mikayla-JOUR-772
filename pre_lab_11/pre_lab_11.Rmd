...
**Answer**
**It reads all of the press releases from Cardin's site and organizes them by date, title, url, and the contents of the release**
```{r}
...
**Answer**
**It shows all of the unique words and their occurences**
```{r}
...
**Answer**
**It changed the nested results of unique and made them a dataframe we can call and view**
```{r}
...
**Answer**
**Not really, the words that are being counted are common words like 'the' which don't really show us anything interesting about the press releases**
...
**Answer**
**It's definitely more interesting because the words aren't everyday words, for example 'infrastructure', so they can actually provide context to the trends of the press releases**
```{r}
unique_words |>
...
**Answer**
**It show the unique words, the number of times they appear, and the percentage of it's occurrence out of all the words**
```{r}
unique_words_2021 |>
  anti_join(stop_words) |>
  group_by(word) |>
...
**Task** Run the following code and describe the results.
**Answer**
**It shows a list of the unique phrases or pair of words along with their frequency and overall rate of frequency compared to all of the unique phrases**
```{r}
releases |>
  filter(date < '2022-01-01') |>
...
**Task** Run the following code and describe the results.
**Answer**
**It narrows the phrases down to only those that occurred in 2022 instead of everything leading up to 2022**
```{r}
releases |>
...
**Task** Run the following code and describe the results.
**Answer**
**It shows the common phrases for this year in order of most frequent to least frequent**
```{r}
releases |>
  filter(date >= '2023-01-01') |>
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
...
**Answer**
**It's not perfect, for example cancer isn't inherently a negative thing depending on the context, however it generally does a decent job of categorizing the results**
```{r}
...
bing_word_counts_2021
bing_word_counts_2022
bing_word_counts_2023
```

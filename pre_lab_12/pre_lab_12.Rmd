...
**Answer** 
**They are divided on the website as indiviual entries so the program read each entry as as individual nested list - so by only pulling the first dataframe, we only get the most recent entry which is November 22nd. Also the columns are both character coloumns with unclean names.**
```{r}
...
**Answer** 
**Knowing the location its based on or the name of the contact would be helpful.**
```{r}
...
**Answer** 
**If you replace <li> with <ul>, you only get 7 entries.**
```{r}
...
**Answer** 
**It provides a simple title for each entry that gives us an idea of what the source of the entry is.**
```{r}
...
**Answer** 
**All 122 entries are in a table under a "value" column.**
```{r}

...
**mutate(month = month(date, label=TRUE)) adds a "month" column to the dataset and assigns the months abbreviations. group_by(month) groups the data by month. summarize(total_calls = n()) summarizes the total number of calls within each month. arrange(desc(total_calls)) arranges the data in descending order based on the total number of calls for that moonth.**
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
**a bar graph plotting the number of calls by month**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
**the months are arranged in ascending order by total number of calls instead of by month**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
**the graph is arranged by number of calls per month arranged in ascending order by total calls**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
**the background is white instead of grey**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**the background is blue instead of white and there is a black border at the bottom of the graph**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer**
**the graph and axes are titled and the graph cites the source at the bottom**
```{r}
baltco_911_calls_by_month |>
  ggplot() +
  geom_bar(aes(x=reorder(month,total_calls), weight=total_calls)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
**the dataset lists the full date instead of the month**
```{r}
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
**a line graph plotting the umer of calls per day from month to month**
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
**every plot point is labeled on the x-axis instead of just the months and year**
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer**
**the dates are angled which makes a little more space for them**
```{r}
baltco_911_calls_by_date |>
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 

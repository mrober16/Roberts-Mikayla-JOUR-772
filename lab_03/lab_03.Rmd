...
# Using janitor, change the column names so that they are lower-cased and any spaces are replaced with underscores
...
pg_schools <- cleaned_payments |>
 filter(vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOLS" | vendor_name == "PRINCE GEORGES COUNTY SCHOOL" | vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOL" | vendor_name == "PRINCE GEORGES PUBLIC SCHOOLS")
```
...
A2:**2021 had the largest amount with a total of $1,470,133,533** **I find it odd that 2010 has the smallest total, especially compared to 2009 and 2008.**

```{r}
pg_schools <- pg_schools |> mutate(date=dmy_hms(date))
...
A3.**combine the ones that appear similarly into one identical vendor name**

```{r}
capital_projects <- cleaned_payments |> filter(agency_name == "CAPITAL PROJECTS- UNIVERSITY OF MARYLAND")

capital_projects |> group_by(vendor_name) |> summarize(total_amount = sum(amount))
```
...
A4:
**Stem cell research was funded for the first and only time in 2020 with a total of $40,913.65**
```{r}
capital_projects |>
  filter(vendor_name == "STEMCELL TECHNOLOGIES INC.") |>
...
A5:
**The data shows that there are 17 grantees however, there are probably less considering there are a few duplicates that were not grouped together due to their difference in verbiage and format. It is interesting that the Division of Research received the least amount of grant money consdiering that the University of Maryland prides itself on its researhc opportunties.**
```{r}
cleaned_md_grants_loans |>
  filter(zip_code == "20742") |>
  group_by(grantee) |>
  summarize(total_received = sum(amount)) |>

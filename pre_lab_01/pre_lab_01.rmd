...
**Answer**
umd_courses |> **use this data and then**
  group_by(term) |> **group it by term and then**
  summarise(     **summarize it by the amount of classes**
    count_classes = n()
...
**arrange(desc(count_classes)) sorted the data so it's arranged by the greatest number of classes to the least instead of ascending chronological order**
...
**Answer**
**the data is grouped by department and term since group_by(term, department) |> was added to the codeblock**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**
group_by(term) |>  **the data is grouped by term**
    total_seats = sum(seats)**the data summarizes the total number of seats the classes of a term**
  ) |>
  arrange(desc(total_seats)) **the data is arranged by the number of seats from greatest to least**
...
**the top 2 terms both have an average of 46 seats**
    mean_seats = mean(seats),
    median_seats = median(seats),
    # add minimum seats here
    # add maximum seats here**the data is being summarized by the average and median number of seats in the classes in a term using the largest and smallest number of seats**
...
**Answer**
group_by(department) |>**the data is grouped by department instead of term so the program is summarizing the number of classes, and the total number of seats in those classes, offered by each department**
**the data is sorted by the total number of seats in the classes from greatest to least**
### Task 12: Questions
...
1.  **Is there regulation on the average number of seats each department should offer in their courses?**

2.  **What encouraged a drastic increase of seats after Aug. 2018?**

3.  **Is there a reason that there were only 2 journalism classes in Dec. of 2017?**

...
**Warning: One or more parsing issues, call `problems()` on your data frame for details** **When you input problems(), a list of the data that did not compute shows up.**
```{r}
...
**Vendor Name is chr data. Amount is dbl data. Date is chr data.**
```{r}
glimpse(payments)
```
...
**Answer**
**A total of 549 zip codes don't have 5 characters. I think they may be missing zeros.**
```{r}
payments |>
  group_by(`Vendor Zip`) |>
...
**Answer**
**There is a type of loan category that seems to not have registered in the data set.**
```{r}
md_grants_loans |> 
...
**Answer**
**2009 and 2012 have two of the smallest amount - both being under $100**
```{r}
md_grants_loans |> 
  arrange(Amount)
```
...
**Answer**
**All of the column names are lower case and have an underscore between words instead of a space.**
```{r}
# cleaning function
...
**Answer** 
**All of the data except the category column has all-caps lettering.**
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
**Answer**
**The zip5 column only has the first 5 digits of the original zip codes.**
```{r}
# cleaning function
cleaned_md_grants_loans <- md_grants_loans |>
  clean_names() |> 
...
**Answer**
**21202 received the most money from the Maryland Tourism Board with a total of $648,069 of funding - which is not surprising considering that's the Baltimore area which is high in tourism.**
```{r}
cleaned_md_grants_loans |> 
